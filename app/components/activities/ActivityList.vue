...
const { activities, loading, createActivity, updateActivity, deleteActivity, reorderActivities } = useActivities();
...
onMounted(async () => {
  await activities.value.length === 0 && useActivities().loadActivities(props.protocolId);
...
const protocolActivities = computed(() => {
  return activities.value.filter(a => a.protocolId === props.protocolId);
});
  ...
  try {
    if (editingActivity.value) {
      ...
      const maxOrder = Math.max(
        ...(protocolActivities.value.map(a => a.order) || [0]),
        -1,
      ) + 1;
